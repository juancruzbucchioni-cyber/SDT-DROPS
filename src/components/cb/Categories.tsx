      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 xl:grid-cols-4">
          {categories.map((category, index) => (
            <a key={category.id} href={`#${category.id.startsWith("cat-") ? category.id : `cat-${category.id}`}`} className={`group reveal card-shimmer stagger-${Math.min(index + 1, 7)} overflow-hidden rounded-xl border border-border bg-card transition-all hover:-translate-y-0.5 hover:shadow-[0_10px_30px_rgba(17,24,39,.08)]`}>
