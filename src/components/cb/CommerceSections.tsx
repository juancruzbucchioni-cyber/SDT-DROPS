...
export function TrustSection() {
  return (
    <section id="nosotros" className="border-y border-[#D7DCE3] bg-[#E5E9EE] py-12 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {benefits.map(({ Icon, title, text }, index) => (
            <article key={title} className="group rounded-xl border border-[#D7DCE3] bg-[#F3F5F7] p-4 transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-[0_12px_28px_rgba(17,54,112,.09)] sm:p-6">
...
export function CatalogCta() {
  return (
    <section className="bg-[#E7EAEE] py-12 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        <div className="relative overflow-hidden rounded-2xl bg-[#071632] px-5 py-8 text-white shadow-[0_16px_40px_rgba(7,22,50,.18)] sm:px-10 sm:py-12">
          <div className="relative flex flex-col items-start justify-between gap-7 md:flex-row md:items-center">
            ...
            <a href="#productos" className="inline-flex min-h-12 w-full shrink-0 items-center justify-center gap-2 rounded-lg bg-primary px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-blue-500 sm:w-auto">Explorar catálogo <ArrowRight className="h-4 w-4" /></a>
          </div>
        </div>
      </div>
    </section>
